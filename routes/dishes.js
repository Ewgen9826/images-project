...
router.get('/:page', Guard, async (req, res) => {
    try {
        ...
        if(req.query.catfilter == undefined && req.query.cuifilter == undefined){

            Dish.find({}).skip(perPage * page - perPage)
            .limit(perPage)
            .then(dishes => {
                Dish.count().then(count => {
...
router.post('/uploads', Guard, upload.array("file", 100), async (req, res) => {
    ...
    try {
        await dish.save();
        res.status(201).redirect('/dishes/1');
